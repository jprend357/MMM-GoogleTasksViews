...
const DEFAULT_HIGHLIGHT_STARRED_TASKS = true
const DEFAULT_STARRED_TITLE_PREFIXES = ["* ", "\u2605 ", "\u2B50 "]
...
const STARRED_TASK_FIELDS = ["starred", "isStarred", "favorite", "important"]

function detectStarredTaskField(task) {
  return STARRED_TASK_FIELDS.some(field => task[field] === true)
}

function stripStarredTitlePrefix(title, starredTitlePrefixes = []) {
  const matchingPrefix = starredTitlePrefixes.find(prefix => prefix && title.startsWith(prefix))
  ...
  return {
    isStarred: true,
    title: title.slice(matchingPrefix.length),
  }
}

function normalizeTask(task, options = {}) {
  const title = task.title || "(untitled task)"
  const highlightStarredTasks = options.highlightStarredTasks ?? DEFAULT_HIGHLIGHT_STARRED_TASKS
  const starredTitlePrefixes = options.starredTitlePrefixes || DEFAULT_STARRED_TITLE_PREFIXES
  const hasStarredField = detectStarredTaskField(task)
  const titlePrefixResult = highlightStarredTasks
    ? stripStarredTitlePrefix(title, starredTitlePrefixes)
    : { isStarred: false, title }
  const isStarred = Boolean(highlightStarredTasks && (hasStarredField || titlePrefixResult.isStarred))

  return {
    id: task.id,
    title: titlePrefixResult.title || "(untitled task)",
    status: task.status,
    due: task.due,
    isStarred,
  }
...
async function fetchGoogleTasks(options = {}) {
  ...
  for (const taskList of selectedTaskLists) {
    ...
    const tasks = activeTasks.slice(0, remainingSlots).map(task => normalizeTask(task, options))
...
module.exports = {
  ...
  DEFAULT_HIGHLIGHT_STARRED_TASKS,
  DEFAULT_STARRED_TITLE_PREFIXES,
  DEFAULT_TOKEN_PATH,
  SCOPES,
  fetchGoogleTasks,
  getAuthorizedClient,
